...
| `privacy.spec.js`                | 6     | `privacy.html` renders fully translated in the language set via `cardLanguage` (English (UK), Spanish, French, Portuguese (PT), German), and falls back to English (UK) for "system" or no setting                                                                                                                                                                                                                   |
| `search.spec.js`                 | 6     | Live search filtering, case-insensitivity, clear, ESC clear, no-match, clear-button hit-box regression                                                                                                                                                                                                                                                                                                               |
...
- The clear button's hoverable/clickable area exactly matches its visible bounds - no dead pixel inside it, no drift past its edges when the pointer arrives via continuous motion
...
- Renders in Spanish, French, Portuguese (PT), and German respectively when `cardLanguage` is seeded as `es`, `fr`, `pt-PT`, `de-DE` (via `addInitScript`) before navigating to `/privacy.html`
