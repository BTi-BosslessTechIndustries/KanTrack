...
| `modal.js`           | Task detail modal: `openTaskModal()`, `saveAndCloseModal()`, `closeTaskModal()`, note history render; images restored in parallel via `Promise.all`; refreshes tag filter bar on close                                                            |
...
| `tags.js`            | Tag creation, assignment, color coding (preset palette + custom hex via `<input type="color">`), tag selector render; pinned tags show in a management panel with assign/delete actions; `cleanupUnusedTags` preserves tags used by trashed tasks |
